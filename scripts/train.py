...
def parse_arguments():
    ...
    parser.add_argument(
        "--config", type=str, default="configs/config.yaml", help="Path to configuration file (default: configs/config.yaml)"
    )
...
def print_results_summary(results):
    ...
    logger.info("   • Metrics:       artifacts/metrics/evaluation_report.json")
    logger.info("   • MLflow Runs:   mlruns/")
...
def save_training_summary(results, start_time, end_time):
    ...
    try:
        ...
        with open(summary_file, "w") as f:
            ...
            f.write("  - Models: artifacts/models/\n")
            f.write("  - Metrics: artifacts/metrics/\n")
